...
Kurator will reconcile Phase 5 objects via the existing **mqweb `/mqsc`** path
...
`SET CHLAUTH` portion is the Phase 5 target.
...
## `MQAdmin` port extensions (Phase 5)

The port already includes topic and SVRCONN channel operations (Phase 4).
Phase 5 adds illustrative auth methods:

```go
// Illustrative — signatures to be finalized in Phase 5.
...
those objects; they document the Phase 5 target state and test `RunMQSC` plumbing.
