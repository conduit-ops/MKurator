...
How to upgrade the Kurator operator between releases without surprising CRD,
...
# Helm-only clusters: kubectl apply --server-side -f charts/kurator/crds/

# 2. Operator
kubectl apply -f install.yaml
# or: helm upgrade --install kurator … --version "${VERSION}"

kubectl -n kurator-system rollout status deployment/kurator-controller-manager
kubectl -n kurator-system wait --for=condition=Available certificate/webhook-server-cert --timeout=120s
...
| **0.2.x** | **0.3.0+** | Module and image registry moved to `konih/kurator` ([ADR-0006](adr/0006-project-name-kurator.md)). Update `image.repository` / install manifest URLs. |
...
Kurator CRDs are generated from kubebuilder markers and shipped in release assets
(`install-crds.yaml`) and [`charts/kurator/crds/`](../charts/kurator/crds/).
...
- **Helm** installs CRDs on first install only; upgrading the chart does not always
  refresh CRDs. Re-apply `install-crds.yaml` or `charts/kurator/crds/` explicitly when
...
kubectl explain queue.spec --api-version=messaging.kurator.dev/v1alpha1
```

## Validating webhooks and cert-manager

With `webhooks.enabled=true` (Helm default), the API server calls Kurator’s
...
Kurator does **not** bundle cert-manager. You must install it in the cluster
...
Upgrade cert-manager on its own lifecycle **before** or **in parallel with** Kurator
only when release notes require a newer API; otherwise keep cert-manager stable and
upgrade Kurator independently.
...
2. Check webhook cert: `kubectl -n kurator-system describe certificate webhook-server-cert`.
3. After renewal, the controller pod should continue running; if webhooks fail with
   TLS errors, restart the deployment once:  
   `kubectl -n kurator-system rollout restart deployment/kurator-controller-manager`.
...
out with the pinned `ghcr.io/konih/kurator:<version>` image.
...
defaults from [charts/kurator/README.md](../charts/kurator/README.md) when release notes
call them out.
...
- [charts/kurator/README.md](../charts/kurator/README.md) — Helm values reference  
