...
Vendored and cloned samples under [`references/`](../references/) support Kurator
development. They are **not** part of the shipped operator. Kurator does **not**
...
## Kurator vs IBM MQ Operator

| | IBM MQ Operator + gitops samples | Kurator |
|---|----------------------------------|---------|
| **Goal** | Install and operate Queue Managers | Reconcile queues, topics, channels on an existing QM |
| **Primary CR** | `QueueManager` (`mq.ibm.com/v1beta1`) | `Queue`, `Topic`, `Channel`, `QueueManagerConnection` (`messaging.kurator.dev/v1alpha1`) |
...
| [`references/mq-gitops-samples/queue-manager-basic-deployment/`](../references/mq-gitops-samples/queue-manager-basic-deployment/) | **High** | MQSC examples (channels, CHLAUTH), secret key conventions, sample `QueueManager` YAML showing bootstrap MQSC. Mirrored in [`test/e2e/fixtures/`](../test/e2e/fixtures/). **Do not** adopt ConfigMap-at-QM as Kurator’s primary model. |
| [`references/mq-gitops-samples/queue-manager-deployment/`](../references/mq-gitops-samples/queue-manager-deployment/) | **Low–medium** | ArgoCD/Kustomize patterns only if adding GitOps examples for **Kurator CRs**. “Dynamic MQSC” ConfigMaps are a contrast, not a target design. |
...
| [`references/cluster-setup/`](../references/cluster-setup/) (other) | **Skip** | Strimzi, GCP, PocketIDP — unrelated to Kurator. |
...
For deploying a QM when not using kind, see [Deploying a queue manager for Kurator](DEVELOPMENT.md#deploying-a-queue-manager-for-kurator) in DEVELOPMENT.md.
